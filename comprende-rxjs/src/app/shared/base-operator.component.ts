...
import { capitalizeFirstLetter } from 'src/app/utils/strings';
...
export abstract class BaseOperatorComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  public ngOnInit() {
    this.titleService.setTitle(
      `${capitalizeFirstLetter(this.operatorName)} rxjs`
    );
    this.metaService.updateTag({
      name: 'description',
      content: `Explicación del operador rxjs ${this.operatorName}`,
    });
  }
  ...
  protected abstract isElementDeliveredToOperator(
    e: ElementInConveyor
  ): boolean;
  protected abstract isElementDeliveredToSubscriber(
    e: ElementInConveyor
  ): boolean;
  ...
  public onSubscribe(isSubscription: boolean): void {
    Object.values(this.controllerButtons).forEach((buttons) =>
      buttons.forEach((button) => (button.enabled = isSubscription))
    );
    ...
    if (!isSubscription) {
      ...
      this.demoSubscription = this.elementReachesOperator$
        .pipe(this.operator)
        .subscribe({
  ...
  public onControllerButtonClick(button: ButtonController) {
    this.addElementToBeginningOfConveyor(
      button.controllerId,
      button.type,
      button.value
    );
  ...
  protected abstract addElementToBeginningOfConveyor(
    conveyorId: string,
